...
for epoch in range(epochs):
    for batch in minibatch(train_data, size=compounding(4, 16, 1.001)):
      ...
      },
      {
        ...
        label: "Score",
        agent: "LightGBM",
        desc: "A calibrated LightGBM predicts 90-day job retention. Optuna tunes it, monotonic constraints encode domain knowledge, and Platt scaling keeps the probabilities honest. Automated leakage detection blocks training on contaminated data.",
  ...
  },
  {
    ...
    stages: [
      ...
      },
      {
        id: "classify",
        label: "Classify",
        desc: "Claude API now, a custom first-stage classifier later. The phased path is projected to cut inference cost by about 95%.",
        tech: ["LLM API", "Custom model later"],
      },
      {
        id: "record",
        label: "Record",
        desc: "ICC-aligned incident records: a violation-type taxonomy, geographic and temporal metadata, and source verification for chain of custody.",
        tech: ["ICC standards"],
      },
      {
        id: "review",
        label: "Review",
        desc: "Human review before anything publishes, immutable audit trails, and row-level security across three sensitivity tiers, with Taliban intelligence services in the threat model. Every verified incident also labels training data for the domain-specific model planned next.",
