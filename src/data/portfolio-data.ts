export const portfolioData = {
  personal: {
    name: "Mohammad Hadi Yaqoobi",
    title: "IT Business & Systems Analyst · Data & Automation Enthusiast",
    subtitle: "Bridging business requirements with technical solutions using SQL, Python, dashboards, and APIs",
    location: "Boston, MA (Green Card Holder)",
    focus: ["Data Science", "Systems Analysis", "Automation"],
    email: "your.email@example.com",
    linkedin: "https://linkedin.com/in/yourprofile",
  },

  about: {
    description: `I have a mixed background in data, systems analysis, and operations. My strength lies in translating complex business problems into technical solutions that drive efficiency and insights. Whether it's automating SQL workflows, building performance dashboards, or documenting system requirements, I thrive at the intersection of business and technology.`,
    education: "Aviation Management + Management Studies + Data Science & Computer Science Courses",
    interests: ["Data Science", "IT BSA", "Analytics", "Automation", "Product & Systems"],
  },

  education: [
    {
      degree: "Master of Science in Management Studies",
      field: "Data Analytics (STEM)",
      institution: "Boston University - Questrom School of Business",
      logo: "/logos/boston-university.png",
      year: "2023",
      description: "STEM Master's degree with a focus on data analytics, business intelligence, and quantitative decision-making.",
    },
    {
      degree: "Bachelor of Science",
      field: "Aviation Management",
      institution: "Kocaeli University",
      logo: "/logos/kocaeli.png",
      year: "2019",
      description: "Comprehensive program covering aviation operations, management principles, and industry regulations.",
    },
    {
      degree: "MicroBachelor",
      field: "Computer Science",
      institution: "New York University (NYU)",
      logo: "/logos/nyu.png",
      year: "2022",
      description: "Focused curriculum in programming fundamentals, algorithms, and software development.",
    },
  ],

  timeline: [
    {
      year: "2024 – 2025",
      title: "IT Business & Systems Analyst",
      company: "Equity Residential",
      location: "Boston, MA",
      description: "Focus: SQL automation, API testing, rent-reporting workflows, fee-transparency dashboards",
      highlights: [
        "Automated weekly/monthly rent delinquency reports using stored procedures and scheduling",
        "Collaborated with DevOps/QA and business stakeholders to improve data quality and workflows",
        "Built or supported dashboards for fee transparency and performance tracking",
      ],
      type: "current",
    },
    {
      year: "Previous Experience",
      title: "Data Analytics & Operations",
      company: "Various Roles",
      location: "",
      description: "Work with SQL, Python, Excel, and BI tools",
      highlights: [
        "Experience with As-Is / To-Be workflows, BRD/FRD documentation",
        "Stakeholder communication and requirements gathering",
        "Data analysis and process improvement initiatives",
      ],
      type: "past",
    },
    {
      year: "Future",
      title: "Data & AI-Driven Product Leader",
      company: "Vision",
      location: "",
      description: "Growing toward advanced data science, AI, and product-centric roles",
      highlights: [
        "Leveraging data science and machine learning for business impact",
        "Building intelligent systems and data-driven products",
        "Leading cross-functional teams in analytics and automation",
      ],
      type: "future",
    },
  ],

  skills: [
    {
      category: "Data & Programming",
      icon: "Database",
      modules: [
        "SQL (Stored Procedures, Joins, Validation)",
        "Python",
        "APIs & JSON",
        "Data Validation",
      ],
    },
    {
      category: "Analytics & Visualization",
      icon: "BarChart3",
      modules: [
        "Power BI",
        "Tableau",
        "Excel Advanced",
        "KPI Dashboards",
        "Performance Tracking",
      ],
    },
    {
      category: "Systems & Process",
      icon: "GitBranch",
      modules: [
        "IT Business & Systems Analysis",
        "Workflow Mapping",
        "As-Is / To-Be Analysis",
        "BRDs & FRDs",
      ],
    },
    {
      category: "Tools & Methods",
      icon: "Wrench",
      modules: [
        "Jira",
        "Azure DevOps",
        "Agile/Scrum",
        "UX Wireframes",
        "Process Diagrams",
      ],
    },
  ],

  projects: [
    {
      title: "SQL Automation System",
      description: "Automated rent reminder reports using stored procedures and scheduling",
      tech: ["SQL", "Stored Procedures", "Scheduling", "Reporting"],
      highlights: [
        "Reduced manual reporting time by 80%",
        "Improved data accuracy and consistency",
        "Streamlined stakeholder communication",
      ],
      github: "#",
      demo: "#",
    },
    {
      title: "Data Pipeline & API Integration",
      description: "Ingesting JSON from external API into SQL tables",
      tech: ["Python", "APIs", "JSON", "SQL", "ETL"],
      highlights: [
        "Built scalable data ingestion pipeline",
        "Implemented error handling and logging",
        "Automated data validation processes",
      ],
      github: "#",
      demo: "#",
    },
    {
      title: "Fee Transparency Dashboard",
      description: "Building dashboards to support fee transparency and resident metrics",
      tech: ["Power BI", "SQL", "DAX", "Data Modeling"],
      highlights: [
        "Visualized complex fee structures for stakeholders",
        "Enabled data-driven decision making",
        "Improved regulatory compliance tracking",
      ],
      github: "#",
      demo: "#",
    },
  ],
};
